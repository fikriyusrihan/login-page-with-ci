...
class Auth extends CI_Controller
{
        ...
        if ($user) {
            if ($user['is_active'] == 1) {
                if (password_verify($password, $user['password'])) {
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email or Password not match with our database!</div>');
                    redirect('auth');
                }
            } else {
    ...
    public function registration()
    {
        ...
        $this->form_validation->set_rules('password1', 'Password', 'required|trim|min_length[4]|matches[password2]', [
            ...
            'min_length' => "Password at least must have 4 character!"
        ]);
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Your account has been created. Please activate it!</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'automail.fikri@gmail.com',
            'smtp_pass' => 'th1550c14l',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('automail.fikri@gmail.com', 'Fikri Yusrihan');
    ...
    public function resetPassword()
    {
        ...
        if ($user) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Reset password failed, email not registered!</div>');
            redirect('auth');
        }
    }
